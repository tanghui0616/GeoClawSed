...
! =========================================================================
    program amr2
        ...
        use amr_module, only: dbugunit, parmunit, outunit, inunit, matunit ! Parameters and variables related to I/O and checkpointing
        use amr_module, only: mxnest, rinfinity, iinfinity ! data structure info
        use amr_module, only: xupper, yupper, xlower, ylower !domain description variables
        use amr_module, only: hxposs, hyposs, intratx, intraty, kratio !data structure info
        use amr_module, only: cfl, cflv1, cflmax, evol !method parameters

        use amr_module, only: checkpt_style, checkpt_interval, tchk, nchkpt !checkpointing
        use amr_module, only: rstfile ! Restart file name

        use amr_module, only: max1d, maxvar, maxlv !data structure info

        use amr_module, only: method, mthlim, use_fwaves, numgrids !data structure info
        use amr_module, only: nghost, mwaves, mcapa, auxtype !nghost (domain description variables) other method parameters
        use amr_module, only: tol, tolsp, flag_richardson, flag_gradient!tol, tolsp:data structure info; other method

        use amr_module, only: nghost, mthbc !domain description variables
        use amr_module, only: xperdom, yperdom, spheredom !domain description variables

        use amr_module, only: nstop, nout, iout, tfinal, tout, output_style !Parameters and variables related to I/O and checkpointing
        use amr_module, only: output_format, printout, verbosity_regrid !Parameters and variables related to I/O and checkpointing
        use amr_module, only: output_q_components, output_aux_components !Parameters and variables related to I/O and checkpointing
        use amr_module, only: output_aux_onlyonce, matlabu!Parameters and variables related to I/O and checkpointing

        use amr_module, only: lfine, lentot, iregridcount, avenumgrids!lfine:data structure info and other collect stats
        use amr_module, only: tvoll, rvoll, rvol, mstart, possk, ibuff!data structure info and collect stats
        use amr_module, only: timeRegridding, timeUpdating, timeValout!collect stats
        use amr_module, only: timeGrdfitAll,timeFlglvl,timeGrdfit2,timeSetaux,timeFilval!collect stats
        use amr_module, only: timeBound, timeStepgrid, timeFlagger,timeBufnst,timeFilvalTot!collect stats
        use amr_module, only: kcheck, iorder, lendim, lenmax

        use amr_module, only: dprint, eprint, edebug, gprint, nprint, pprint ! Debugging flags (verbose output)
        use amr_module, only: rprint, sprint, tprint, uprint! Debugging flags (verbose output)

        use amr_module, only: t0, tstart_thisrun !Parameters and variables related to I/O and checkpointing

        use regions_module, only: set_regions !for region refinement
        use gauges_module, only: set_gauges, num_gauges ! Parameters and variables related to gauges
